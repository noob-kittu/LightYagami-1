...
async def chatbot_function(client, message):
    ...
    if chat_id in en_chats:
        test = msg
        test = test.replace("johan", "Liebert")
        test = test.replace("Johan", "Liebert")
        response = await lunaQuery(
            test, message.from_user.id if message.from_user else 0
        )
        response = response.replace("Liebert", "Johan")
        response = response.replace("liebert", "Johan")
...
@eren.on_message(
    filters.regex("Johan|johan|Johan Liebert|johan liebert|")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
...
__help__ = """
 Chatbot utilizes the Branshop's API and allows Johan to talk and provides a more interactive group chat experience.
